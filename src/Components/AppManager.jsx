...
function ProductManager() {
  ...
  const ACCESS_KEY = "$2a$10$lHC6.TYTGJdHEzvNt8D6DOCWIDRJHjfUUWMBzLBRfhQGlEBEIK6oa";
  ...
  const fetchProductById = async (id) => {
    ...
    try {
      ...
      const productData = response.data.record.Products.find(p => p.id === parseInt(id));
  ...
  const updateProduct = async (productId, updatedProductData) => {
    ...
    try {
      ...
      const updatedProducts = currentProducts.map(product => 
        product.id === parseInt(productId) ? { ...product, ...updatedProductData } : product
      );
  ...
  const deleteProduct = async (productId) => {
    ...
    try {
      ...
      // حذف المنتج
      const updatedProducts = currentProducts.filter(product => product.id !== parseInt(productId));
  ...
  const handleInputChange = (e) => {
    ...
    setNewProduct({
      ...newProduct,
      [name]: name === 'price' || name === 'OldPrice' ? parseFloat(value) : value
    });
  };
  ...
  return (
    <div dir="rtl" className="container mx-auto p-4 font-sans">
      <h1 className="text-2xl font-bold mb-4 text-center">إدارة المنتجات مع JSONBin.io</h1>
      
      {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">{error}</div>}
      {responseMessage && <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">{responseMessage}</div>}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {products.map(product => (
                  <div key={product.id} className="border p-3 rounded hover:bg-gray-50">
                    <p><strong>رقم:</strong> {product.id}</p>
                    <p><strong>الاسم:</strong> {product.name}</p>
                    <p><strong>السعر:</strong> {product.price}</p>
                    <p><strong>السعر القديم:</strong> {product.OldPrice}</p>
                    <div className="mt-2">
                      <button 
